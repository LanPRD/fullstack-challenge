...
export function CrashGraph({ phase, multiplier, dataPoints, serverSeedHash, crashPoint }: Props) {
  ...
  const graph = useMemo(() => {
    ...
    const pts = dataPoints.map((p) => ({ x: ptX(p.elapsed, maxE), y: ptY(p.multiplier, maxM) }));
  ...
  return (
    <div
      ...
      style={{ background: "#07070f", minHeight: 200 }}
    >
      {/* Hash chip */}
      ...
      <div className="absolute inset-0 flex flex-col items-center justify-center z-10 pointer-events-none">
        {phase === "BETTING" && (
          <div className="text-center">
            <div className="text-yellow-400 text-sm font-semibold animate-pulse mb-1">
              FASE DE APOSTAS
            </div>
        ...
        {phase === "CRASHED" && (
          <div className="text-center">
            ...
            <div className="text-6xl font-mono font-bold glow-red" style={{ color: "#ef4444" }}>
              {crashPoint?.toFixed(2) ?? multiplier.toFixed(2)}×
            </div>
          </div>
        )}
        {phase === "IDLE" && (
          <div className="text-gray-500 text-xl">Aguardando rodada…</div>
        )}
      </div>
          ...
          return (
            <g key={tick}>
              ...
              <text x={PAD.left - 6} y={y + 4} textAnchor="end" fill="#4b5563" fontSize="10">
                {tick >= 2 ? `${tick.toFixed(0)}×` : `${tick.toFixed(1)}×`}
              </text>
            </g>
          );
        })}
